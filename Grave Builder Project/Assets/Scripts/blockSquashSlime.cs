...
public class blockSquashSlime : MonoBehaviour
{
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
        if (collision.gameObject.tag.Equals("MediumSlime"))
        {
            ...
            if (collision.transform.position.y+200 < block.position.y && (collision.transform.position.x < block.position.x + 280 && collision.transform.position.x > block.position.x - 280))
            {
                Debug.Log("Destroyed!");
